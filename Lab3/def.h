...
//结点类型
enum NodeKind {
  //抽象语法树结点类型（表达式类型的结点省略直接用yytoken）
  EXT_DEF_LIST, EXT_VAR_DEF, FUNC_DEF, FUNC_DEC, EXT_DEC_LIST,
  PARAM_LIST, PARAM_DEC, VAR_DEF, DEC_LIST, DEF_LIST, COMP_STM,
  STM_LIST, EXP_STMT, IF_THEN, IF_THEN_ELSE, FUNC_CALL, EXP_LIST,
  RETURN_E, SELFADD_R, SELFSUB_R,
  FUNCTION, PARAM, CALL, LABEL, GOTO, JLT, JLE, JGT, JGE, EQ, NEQ,
};

//操作数
struct Operand {
  int kind; //标识操作数结点的类型(标识符ID、常数INT等)
...
struct CodeNode { //采用双向循环链表存放中间语言代码
  int op;   //TAC代码的运算符种类
  struct Operand opn1, opn2, result; //2个操作数和运算结果
...
//符号表
struct SymbolTable {  //顺序栈结构,index初值为0
