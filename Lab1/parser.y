...
%type<ptr> Program ExtDefList ExtDef Specifier ExtDecList FuncDec ArrayDec CompSt VarList VarDec ParamDec Stmt StmList DefList Def DecList Dec Exp ExpList
//%token指定终结符的语义值类型
%token<type_int> INT
...
%token PLUS MINUS STAR DIV ASSIGNOP AND OR NOT IF ELSE WHILE RETURN COMADD COMSUB FOR COMSTAR COMDIV BREAK CONTINUE FOR_E RETURN_E VOID
//结合性，优先级由低到高
...
ExtDef: Specifier ExtDecList SEMI {$$=mknode(EXT_VAR_DEF,yylineno,2,$1,$2);}		//外部(全局)变量定义语句
	...
	| Specifier ArrayDec SEMI {$$=mknode(ARRAY_DEF,yylineno,2,$1,$2);}		//数组定义语句
	| error SEMI {$$=NULL;}
	;
...
ParamDec: Specifier VarDec {$$=mknode(PARAM_DEC,yylineno,2,$1,$2);}
	;
//数组定义语句
ArrayDec: ID LB Exp RB {$$=mknode(ARRAY_DEC,yylineno,1,$3);strcpy($$->type_id,$1);}	//给定了数组大小
	| ID LB RB {$$=mknode(ARRAY_DEC,yylineno,0);strcpy($$->type_id,$1);}	//未给定数组大小
...
Stmt: Exp SEMI {$$=mknode(EXP_STMT,yylineno,1,$1);}
	...
	| FOR LP ExpList SEMI Exp SEMI ExpList RP Stmt {$$=mknode(FOR,yylineno,4,$3,$5,$7,$9);}
...
Def: Specifier DecList SEMI {$$=mknode(VAR_DEF,yylineno,2,$1,$2);}	//不同变量定义
     | Specifier ArrayDec SEMI {$$=mknode(ARRAY_DEF,yylineno,2,$1,$2);}	//数组定义
     ;
...
Exp: Exp ASSIGNOP Exp {$$=mknode(ASSIGNOP,yylineno,2,$1,$3);strcpy($$->type_id,"ASSIGNOP");}//type_id记录运算符
	...
	| SELFADD Exp {$$=mknode(SELFADD_L,yylineno,1,$2);strcpy($$->type_id,"SELFADD");}
	| SELFSUB Exp {$$=mknode(SELFSUB_L,yylineno,1,$2);strcpy($$->type_id,"SELFSUB");}
